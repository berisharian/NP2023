...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    //TODO constructor
        ...
        LogProcessor<T> secondLogProcessor = (logs) ->
                logs.stream().filter(l->l.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new ));

        LogProcessor<T> thirdLogProcessor = (logs) ->
                logs.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
