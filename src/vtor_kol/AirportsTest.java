...
class Flight{
    ...
    public static String formatTime(LocalTime time, int duration){
        ...
        sb.append(String.format("%dh%02dm", hours, duration-(hours*60)));
        return sb.toString();
    }
...
class Airport{
    ...
    public Airport(String name, String country, String code, int passengers) {
        ...
        this.flights=new TreeSet<>(Flight.COMPARATOR);
    }
    public void addFlight(String from, String to, int time, int duration, FlightType flightType){
        flights.add(new Flight(from, to, time, duration, flightType));
    }
...
class Airports{
    ...
    public void addFlights(String from, String to, int time, int duration) {
        airportMap.get(from).addFlight(from, to, time, duration, FlightType.DEPARTURE);
        airportMap.get(to).addFlight(from, to, time, duration, FlightType.ARRIVAL);
    }
    ...
    public void showFlightsFromAirport(String from) {
        ...
        List<Flight> flights = airport.flights.stream()
                .filter(x->x.getType() == FlightType.DEPARTURE)
    ...
    public void showDirectFlightsTo(String to) {
        airportMap.get(to).flights.stream()
                .filter(x->x.getType() == FlightType.ARRIVAL)
