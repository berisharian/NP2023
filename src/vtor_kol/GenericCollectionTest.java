...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    public void addGenericItem(String category, T element) {
        generics.computeIfAbsent(category, x-> new HashSet<>());
      //  generics.computeIfPresent(category, (k, v) -> {v.add(element); return v;});
        generics.get(category).add(element);
    }

    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
        return generics.values().stream()
    ...
    public Collection<T> itemsFromCategories (List<String> categories) {
        return generics.keySet()
                .stream().filter(categories::contains)
                .flatMap(x->generics.get(x).stream())
                .collect(Collectors.toCollection(()->new TreeSet<T>(Comparator.reverseOrder())));
    }
    ...
    public Map<Integer, Long> countByYear(){
        Map<Integer, Long> cc = generics.values().stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(
                        x-> x.getTimestamp().getYear(),
                        Collectors.counting()
                ));
        return new TreeMap<>(cc);
    }
}
