...
class MapOps {
    public static <K extends Comparable<K>, V>  Map<K,V> merge (Map<K,V> first, Map<K,V> second, MergeStrategy<V> mergeStrategy){
        Map<K, V> result=new TreeMap<>(first);
        second.forEach((k,v) -> result.merge(k, v, mergeStrategy::merge));
        return result;
    }
}
